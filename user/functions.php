...
if( $_GET['del_id']!="")
	{
		...
		if(file_exists($company_logo) || file_exists($profile_photo) )
		{
			...
			$rs = mysqli_query($conn,$query);
			$_SESSION['error_msg']  = '<p class="alert alert-danger" role="alert">
		   User Record Has Been Deleted Succesfully.
			</p>';
...
//forgot password msg
	if($_GET['forgot']!="" && $_GET['forgot']=="yes")
	{
		$id = $_GET['user_id'];
		$_SESSION['error_msg']  = '<p class="alert alert-success" role="alert">
		Emails has been sent please check your email for more information!
		</p>';
		header ('location:forgotpassword.php');
    }

//forgot password msg
	if($_GET['forgot']!="" && $_GET['forgot']=="no")
	{
		
		$_SESSION['error_msg']  = '<p class="alert alert-danger" role="alert">
		There is no record against the provided email.
		</p>';
		header ('location:forgotpassword.php');
    }
	?>
